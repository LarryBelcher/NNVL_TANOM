...
if __name__ == '__main__':
	...
	cmd = "python ./tanom_yearly_map.py "+fdate+" "+imgsize
	subprocess.call(cmd,shell=True)

	if(imgsize == '620' or imgsize == '1000' or imgsize == 'DIY'):
		cmd = "python ./tanom_yearly_colorbar.py "+fdate+" "+imgsize
		subprocess.call(cmd,shell=True)
	...
	if(imgsize == 'DIY'):
		...
		img_name = 'tempanomaly-annual-nnvl--'+imgw+'x'+imgh+'--'+yyyy+'-00-00.png'
		tifimg_name = 'tempanomaly-annual-nnvl--'+imgw+'x'+imgh+'--'+yyyy+'-00-00.tif'
		cmd = 'mv '+im1+' '+img_name
		subprocess.call(cmd,shell=True)
		...
		subprocess.call(cmd,shell=True)
		origfile = glob.glob('./Images/Yearly/Orig/ANOM.yearly.*'+yyyy+'*.color.png')[0]
		subprocess.call(cmd,shell=True)
		...
		subprocess.call(cmd,shell=True)
		cmd1 = 'zip tempanomaly-annual-nnvl--'+imgw+'x'+imgh+'--'+yyyy+'-00-00.zip '+img_name+' '+tifimg_name+' '+cbar_name+' noaa_logo.eps '
	...
	if(imgsize != 'DIY'):
		...
		subprocess.call(cmd,shell=True)
	if(imgsize == 'DIY'):
		cmd = 'rm tempanomaly-annual-nnvl*'
		subprocess.call(cmd,shell=True)