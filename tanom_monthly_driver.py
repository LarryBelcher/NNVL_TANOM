...
if __name__ == '__main__':
	...
	cmd = "python ./tanom_monthly_map.py "+fdate+" "+imgsize
	subprocess.call(cmd, shell=True)


	if(imgsize == '620' or imgsize == '1000' or imgsize == 'DIY'):
		cmd = "python ./tanom_monthly_colorbar.py "+fdate+" "+imgsize
		subprocess.call(cmd,shell=True)
	...
	if(imgsize == 'DIY'):
		...
		img_name = 'tempanomaly-monthly-nnvl--'+imgw+'x'+imgh+'--'+yyyy+'-'+mm+'-00.png'
		tifimg_name = 'tempanomaly-monthly-nnvl--'+imgw+'x'+imgh+'--'+yyyy+'-'+mm+'-00.tif'
		cmd = 'mv '+im1+' '+img_name
		subprocess.call(cmd,shell=True)
		...
		subprocess.call(cmd,shell=True)			
		origfile = glob.glob('./Images/Monthly/Orig/ANOM.monthly.*'+yyyy+mm+'*.color.png')[0]
		subprocess.call(cmd,shell=True)
		...
		subprocess.call(cmd,shell=True)
		cmd1 = 'zip tempanomaly-monthly-nnvl--'+imgw+'x'+imgh+'--'+yyyy+'-'+mm+'-00.zip '+img_name+' '+tifimg_name+' '+cbar_name+' noaa_logo.eps '
	...
	#Now for cleanup
	if(imgsize != 'DIY'):
		cmd = 'rm temporary*.png'
		subprocess.call(cmd,shell=True)
	if(imgsize == 'DIY'):
		cmd = 'rm tempanomaly-monthly-nnvl*'
		subprocess.call(cmd,shell=True)